...
# 6. 버튼 클릭 콜백: 플래그만 설정 (쿨타임은 성공 후에 적용)
def start_analysis():
    st.session_state.is_running = True
    st.session_state.error_message = None  # 이전 에러 초기화

# 7. 웹 UI 구성
st.title("📈 AI 단타 분석기 (V3.3)")
...
if st.session_state.is_running:
    ...
    with st.spinner(f"[{ticker}] 상세 지표 및 거래량 분석 중..."):
        ...
        try:
                ...
                try:
                    ...
                    if ("429" in err_str or "RESOURCE_EXHAUSTED" in err_str):
                        ...
                        if attempt < 2:
                            time.sleep(15)
                            continue
                    raise api_err  # 429가 아닌 에러는 즉시 raise
            ...
            if "PerDay" in err_str or "daily" in err_str.lower():
                ...
                st.session_state.error_message = "⏳ API 요청 한도에 도달했습니다. 1분 후 다시 시도해주세요."
            else:
    ...
    # 쿨타임 중: 버튼 비활성화 + 클라이언트 JS 카운트다운 (서버 블로킹 없음)
    ...
    components.html(f"""
        <div id="cooldown" style="
            ...
            color: #31708f;
        ">
            ⏳ 글로벌 쿨타임 중입니다. <strong><span id="sec">{remaining}</span>초</strong> 후 활성화됩니다.
        </div>
        <script>
            let sec = {remaining};
            ...
            const timer = setInterval(() => {{
                ...
                if (sec <= 0) {{
                    clearInterval(timer);
                    cd.innerHTML = '✅ 쿨타임 해제! 분석 버튼이 활성화되었습니다.';
                    cd.style.backgroundColor = '#e8f5e9';
                    cd.style.color = '#2e7d32';
                    // ★ 페이지 새로고침 없이 버튼 disabled 속성만 직접 제거
                    const btns = window.parent.document.querySelectorAll('button');
                    btns.forEach(btn => {{
                        if (btn.innerText.includes('제미니')) {{
                            btn.disabled = false;
                            btn.style.opacity = '1';
